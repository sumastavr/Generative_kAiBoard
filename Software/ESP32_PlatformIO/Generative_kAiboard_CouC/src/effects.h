...
void confetti() {
  ...
  for (byte i = 0; i < 2; i++) {
    int xpos=random16(kMatrixWidth);
    int ypos=random16(kMatrixHeight);
    //Serial.println(XY(xpos,ypos));
    //leds[XY(xpos, ypos)] = ColorFromPalette(currentPalette, random16(255), 255); //CHSV(random16(255), 255, 255);
    leds[XY(xpos, ypos)] = CHSV(random16(255), 255, 255);
...
void scrollText(byte message, byte style, CRGB fgColor, CRGB bgColor) {
  ...
  bitBufferPointer++;
  if (bitBufferPointer > 47) bitBufferPointer = 0;
...
void todoList(){
  uint8_t banner[]={0,1,2,3,8,11,15,19,25,28,32,36,40};
  uint8_t done[]={9,10,13,16,17,18,20,21,22,23,24,29,31,33,34,35,37};
  uint8_t hold[]={};
  uint8_t nope[]={4,5,6,7,10,14,26,27,30,38,39,41,42};

    
    for (int x=0;x<sizeof(banner)/sizeof(banner[0]);x++){
      if(currentEffect==banner[x]){
        scrollText(currentEffect, RAINBOW, 0, CRGB::Black); 
      }
    }

    for (int x=0;x<sizeof(done)/sizeof(done[0]);x++){
      if(currentEffect==done[x]){
         scrollText(currentEffect, NORMAL, CRGB::Green, CRGB(0,0,8));
      }
    }

    for (int x=0;x<sizeof(nope)/sizeof(nope[0]);x++){
      if(currentEffect==nope[x]){
        scrollText(currentEffect, NORMAL, CRGB::Red, CRGB::Black);
      }
    }

    for (int x=0;x<sizeof(hold)/sizeof(hold[0]);x++){
      if(currentEffect==hold[x]){
        scrollText(currentEffect, NORMAL, CRGB::Blue, CRGB::Black);
      }
    }
